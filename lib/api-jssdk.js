...
module.exports = (app, name) => {
  ...
  class ApiJssdk {
    ...
    async getJsapiTicket() {
      ...
      const cacheData = await redis.get(app.config.wx[name].redisPrefix + '-jsapiTicket');
      if (cacheData === null) {
    ...
    async getJsapiTicketFromApi(accessToken) {
      ...
      const res = await app.curl(
        app.config.wx[name].apiBaseUrl + app.config.wx[name].apiUrl.getJsapiTicket,
        {
          dataType: 'json',
          data: {
      ...
      await redis.set(app.config.wx[name].redisPrefix + '-jsapiTicket', res.data.ticket, 'EX', res.data.expires_in);
