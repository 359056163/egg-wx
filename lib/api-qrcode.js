    ...
    async createQRCode(type, sceneValue, expire_seconds = 30) {
      ...
      switch (type) {
        case 'QR_SCENE':
          ...
          postData.action_info.scene = { scene_id: parseInt(sceneValue) };
          break;
        case 'QR_STR_SCENE':
          postData.expire_seconds = expire_seconds;
      ...
      const url = app.config.wx[name].apiBaseUrl + app.config.wx[name].apiUrl.createQRCode + '?access_token=' + accessToken;
