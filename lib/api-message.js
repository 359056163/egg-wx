...
module.exports = (app, name) => {
  ...
  class ApiMessage {
    ...
    async messageEncrypt(msg) {
      if (app.config.wx[name].EncodingAESKey.length !== 43) {
        throw new Error('EncodingAESKey 设置错误');
      }
      const be = new Buffer(4);
      be.writeInt32BE(msg.length, 0);
      const randomMsg = this.getRandomStr() + be.toString() + msg + app.config.wx[name].AppID;
      const padMsg = pkcs7.encode(randomMsg);
      const key = new Buffer.from(app.config.wx[name].EncodingAESKey + '=', 'base64');
    ...
    async sendTemplateMessage(openid, template_id, data, jumpUrl = '', miniprogram = null) {
      ...
      const url = app.config.wx[name].apiBaseUrl + app.config.wx[name].apiUrl.messageTemplateSend + '?access_token=' + accessToken;
      const postData = {
        touser: openid,
        template_id,
        data,
    ...
    checkSign(urlQuery, encryptText) {
      const tmpArray = [ app.config.wx[name].token, urlQuery.timestamp, urlQuery.nonce, encryptText ];
      tmpArray.sort();
    ...
    getSign(timestamp, nonce, encryptText) {
      const tmpArray = [ app.config.wx[name].token, timestamp, nonce, encryptText ];
      tmpArray.sort();
      const sha1 = crypto.createHash('sha1');
      for (const i of tmpArray) {
        sha1.update(i);
